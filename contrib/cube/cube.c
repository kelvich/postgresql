...
g_cube_picksplit(PG_FUNCTION_ARGS)
{
	...
	if (DIM(DatumGetNDBOX(entryvec->vector[FirstOffsetNumber].key)) >= SPLIT_THRESHOLD)
...
_cube_inter(NDBOX *a, NDBOX *b)
{
	...
	cube1 = (DIM(a) > DIM(b)) ? a : b;
	cube2 = (DIM(a) <= DIM(b)) ? a : b;

	result = palloc0(VARSIZE(cube1));
	SET_VARSIZE(result, VARSIZE(cube1));
	SET_DIM(result, DIM(cube1));

	/* compute the intersection */
	for (i = 0; i < DIM(cube2); i++)
	{
		edge1 = Max(
			Min(cube1->x[i], cube1->x[i + DIM(cube1)]),
			Min(cube2->x[i], cube2->x[i + DIM(cube2)])
		);
		edge2 = Min(
			Max(cube1->x[i], cube1->x[i + DIM(cube1)]),
			Max(cube2->x[i], cube2->x[i + DIM(cube2)])
		);
		...
		{
			result->x[i] = edge1;
			result->x[i + DIM(result)] = edge2;
		}
		else
		{
			result->x[i] = 0;
			result->x[i + DIM(result)] = 0;
		}
	}
